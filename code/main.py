...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("main training")
    ...
    parser.add_argument('--n_fold', type=int, default=5, help='n_fold')
    ...
    graph_max_map = {'drugbank':85, 'human':250, 'celegans':250, 'dude':100}
    seq_max_map = {'drugbank': 1200, 'human':3000, 'celegans':3000, 'dude':820}
    graph_max = graph_max_map[DATASET]
    seq_max = seq_max_map[DATASET]

    dir_input = ('../dataset/' + str(DATASET) + '/radius' + str(radius) + '_ngram' + str(ngram) + '/')
    ...
    for fold_num, (train_index, test_index) in enumerate(skf.split(compounds, interactions)):
        ...
        train_cmp = torch.from_numpy(compounds[train_index]).long()
        tmp_val, tmp_test = split_dataset(compounds[test_index], 0.5)
        val_cmp = torch.from_numpy(tmp_val).long()
        test_cmp = torch.from_numpy(tmp_test).long()

        train_cmp_mask = torch.from_numpy(compounds_mask[train_index]).float()
        tmp_val, tmp_test = split_dataset(compounds_mask[test_index], 0.5)
        val_cmp_mask = torch.from_numpy(tmp_val).float()
        test_cmp_mask = torch.from_numpy(tmp_test).float()

        train_adj = torch.from_numpy(adjs[train_index]).float()
        tmp_val, tmp_test = split_dataset(adjs[test_index], 0.5)
        val_adj = torch.from_numpy(tmp_val).float()
        test_adj = torch.from_numpy(tmp_test).float()

        train_protein = torch.from_numpy(proteins[train_index]).long()
        tmp_val, tmp_test = split_dataset(proteins[test_index], 0.5)
        val_protein = torch.from_numpy(tmp_val).long()
        test_protein = torch.from_numpy(tmp_test).long()

        train_protein_mask = torch.from_numpy(proteins_mask[train_index]).float()
        tmp_val, tmp_test = split_dataset(proteins_mask[test_index], 0.5)
        val_protein_mask = torch.from_numpy(tmp_val).float()
        test_protein_mask = torch.from_numpy(tmp_test).float()

        train_interactions = torch.from_numpy(interactions[train_index]).long()
        tmp_val, tmp_test = split_dataset(interactions[test_index], 0.5)
